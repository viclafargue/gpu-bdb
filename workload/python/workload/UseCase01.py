...
def load_data(
    order_path: str, lineitem_path: str, order_returns_path: str
) -> pd.DataFrame:
    ...
    returns_data = lineitem_data.merge(
        order_returns_data,
        how="left",
        left_on=["li_order_id", "li_product_id"],
        right_on=["or_order_id", "or_product_id"],
    )
    raw_data = returns_data.merge(
        order_data, left_on=["li_order_id"], right_on=["o_order_id"]
    )
...
def pre_process(data: pd.DataFrame):
    ...
    groups = (
        data.groupby(["o_customer_sk", "o_order_id"])
        .agg({"row_price": np.sum, "return_row_price": np.sum, "invoice_year": np.min})
        .reset_index()
    )
    ...
    frequency_groups = (
        groups.groupby(["o_customer_sk", "invoice_year"])["o_order_id"]
        .nunique()
        .reset_index()
    )
    frequency = frequency_groups.groupby(["o_customer_sk"]).agg(
        frequency=("o_order_id", np.mean)
    )

    return pd.merge(frequency, ratio, left_index=True, right_index=True)

    x_purchases = pd.DataFrame(
        {"CustomerID": ratios.index, "ReturnRatio": ratios, "Frequency": frequency}
    )
...
def train(featurevector: pd.DataFrame, num_clusters) -> Pipeline:
    ...
    pipeline = Pipeline([("minmax", mms), ("kmeans", km4_purchases)])

    model = pipeline.fit(featurevector[["return_ratio", "frequency"]])

    return model


def serve(model: KMeans, x: pd.DataFrame):
...
def main():
    ...
    if stage == "training":
        start = timeit.default_timer()
        model = train(preprocessed_data, num_clusters)
        end = timeit.default_timer()
