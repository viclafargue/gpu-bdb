...
def preprocess_data(data: pd.DataFrame):
    ...
    data["audio"] = data["audio"].progress_apply(
        resample_audio, desired_sample_rate=SAMPLE_RATE
    )
    # adding silence
    ...
    if "transcript" in data.columns:
        ...
        data["transcript_seq"] = data["transcript_norm"].progress_apply(
            text_to_seq, max_length=None
        )
    ...
    if "transcript" in data.columns:
        ...
        return (
            ...
            x,
            y,
            data["features_len"].values,
            data["labels_len"].values,
        )
    else:
        x = pad_sequences(data["features"].values, dtype=np.float32, padding="post")
...
def main():
    ...
    if stage == "serving":
        model = load_model(
            work_dir / model_file_name, custom_objects={"ctc_dummy": ctc_dummy}
        )
        start = timeit.default_timer()
        ...
        prediction.to_csv(
            output / "predictions.csv", sep=INPUT_SEPARATOR, header=True, index=False
        )
