...
def pre_process(raw_data: pd.DataFrame) -> (np.array, pd.DataFrame):
    ...
    if has_labels:
        ...
        agg_func = {"scan_count": [scan_count, scan_count_abs], "weekday": weekday}
    ...
    weekdays = raw_data.pivot_table(
        index="o_order_id", columns="weekday", values="scan_count", aggfunc=grper
    ).fillna(0.0)
    ...
    departments = raw_data.pivot_table(
        index="o_order_id", columns="department", values="scan_count", aggfunc="sum"
    )
    ...
    final_data: pd.DataFrame = (
        features_scan_count.drop(columns=["weekday"])
        .join(weekdays)
        .join(departments)
        .fillna(0.0)
    )
...
def train(training_data: pd.DataFrame, labels, num_rounds):
    xgboost_clf = XGBClassifier(
        tree_method="hist", objective="multi:softprob", n_estimators=num_rounds
    )
...
def serve(model, data: pd.DataFrame) -> pd.DataFrame:
    ...
    predictions_df = pd.DataFrame(
        {"o_order_id": data.index, "trip_type": dec_fun(predictions)}
    )
