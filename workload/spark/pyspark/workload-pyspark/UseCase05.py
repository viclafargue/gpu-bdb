...
from horovod.spark.common.store import HDFSStore, Store
...
def main():
    ...
    parser.add_argument(
        "--learning_rate", "-lr", required=False, type=float, default=None
    )
    parser.add_argument(
        "--master", metavar="URL", type=str, default=None, help="Spark master url"
    )
    parser.add_argument(
        "--hdfsdriver",
        metavar="DRIVER",
        type=str,
        default="libhdfs",
        help="Name of the HDFS driver",
    )
    parser.add_argument(
        "--namenode",
        metavar="URL",
        type=str,
        help="URL of the HDFS namenode, e.g. domain.tld:8020",
    )
    ...
    if stage == "training":
        ...
        price_suggestions.select("id", "price").write.csv(
            f"{output}/predictions.csv", "overwrite", sep="|", header=True
        )
