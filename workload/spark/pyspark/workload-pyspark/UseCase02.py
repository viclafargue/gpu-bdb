...
from pyspark.sql.functions import lit, max, rand, udf, when
...
def main():
    ...
    if stage == "training":
        ...
        model = train(
            ...
            epochs,
            work_dir,
            spark_horovod.sparkContext.defaultParallelism,
            current_user,
            learning_rate,
        )
        end = timeit.default_timer()
        ...
        preprocessed_data, _ = preprocess_data(
            cleaned_data, max_timesteps=max_timesteps
        )
        end = timeit.default_timer()
        pre_process_time = end - start
        print("pre-process time:\t", pre_process_time)
        ...
        prediction.select("wav_filename", "prediction").withColumnRenamed(
            ...
            f"{output}/predictions.csv", header=True, sep="|"
        )
