...
def make_report(connection: sqlite3.Connection, benchmark_id, include_clean=True):
    ...
    if report["start"] and report["end"]:
        report["duration"] = "{}".format(
            str(
                timedelta(
                    seconds=round(
                        benchmark_meta["end_time"] - benchmark_meta["start_time"], 3
                    )
                )
            )
        )
    ...
    for rec in result.fetchall():
        ...
        phase_name_formatted = (
            f"{rec['phase']}_{rec['phase_run']}"
            if rec["phase"] in phases_with_runs
            else rec["phase"]
        )
        ...
        """
        uc_result = cursor.execute(
            phase_use_case_query,
            (benchmark_id, phase_name, rec["stream"], rec["phase_run"]),
        )
        use_cases = []
        for uc_rec in uc_result.fetchall():
            ...
            uc = {
                "use_case": uc_rec["use_case"],
                "start_time": start_time,
                "end_time": end_time,
                "runtime": uc_rec["runtime"],
                "command": uc_rec["command"],
                "metric_name": uc_rec["metric_name"],
                "metric_value": uc_rec["metric_value"],
                "stream": rename_stream(rec["stream"], phase_name),
                "return_code": uc_rec["return_code"],
                "successful": True if uc_rec["return_code"] == 0 else False,
            }
            use_cases.append(uc)
        phase = {
            "stream": rename_stream(rec["stream"], phase_name),
            "phase": phase_name,
            "phase_run": rec["phase_run"],
            "phase_name_formatted": phase_name_formatted,
    ...
    # per use-case table
    use_case_res = cursor.execute(
        "SELECT * FROM command WHERE benchmark_fk = ? AND sub_phase = 'SubPhase.WORK'",
        (benchmark_id,),
    )
    records = []
    ...
    df["phase_name"] = np.where(
        df["phase"].isin(phases_with_runs),
        df["phase"] + "_" + df["phase_run"].astype(str),
        df["phase"],
    )
    df["phase_name"] = df["phase_name"].str.replace("Phase.", "", regex=False)
    df["phase_name"] = (
        df["phase_name"].str.replace("_nan", "").str.replace(".0", "", regex=False)
    )
    phase_sorting = (
        df.groupby(["phase_name"])["start_time"].min().sort_values().index.values
    )
    df_mean = df.pivot_table(
        values="runtime", index="use_case", columns="phase_name"
    ).reset_index()
    df_sum = df.pivot_table(
        values="runtime", index="use_case", columns="phase_name", aggfunc="sum"
    ).reset_index()
    df = df_sum
...
def get_table(
    ...
    if phase_run:
        filtered_phases = list(
            filter(
                lambda p: p["phase"] == phase and p["phase_run"] == phase_run, phases
            )
        )
    else:
...
def make_graphs(report, output=None):
    ...
    plt.pie(
        phases_runtimes["runtime"],
        labels=phases_runtimes["phase_name_formatted"],
        autopct="%1.1f%%",
    )
    if output:
    ...
    # bar chart
    ax = phases_runtimes.plot.bar(
        x="phase_name_formatted", y="runtime", color=color_cycle, figsize=figsize, rot=0
    )
    ...
    # training
    lst = [
        ucs["use_cases"] for ucs in report["phases"] if ucs["phase"] == "Phase.TRAINING"
    ]
    if len(lst) > 0:
        lst = lst[0]
        lst = list(
            map(lambda uc: {"use_case": uc["use_case"], "runtime": uc["runtime"]}, lst)
        )
        training_runtimes = pd.DataFrame(lst)

        # bar chart
        ax = training_runtimes.plot.bar(
            x="use_case", y="runtime", color=color_cycle, figsize=figsize, rot=0
        )
        ax.set_ylabel("runtime (s)")
        ax.legend("")
        if output:
    ...
    serving_throughput_times = pd.pivot_table(
        serving_throughput_times, values="runtime", index="stream", columns="use_case"
    )
    ...
    use_cases_cols = [
        col
        for col in use_cases_phases.columns
        if col.startswith("TRAINING") or col.startswith("SERVING")
    ]
    use_cases_phases = use_cases_phases[use_cases_cols]
    if len(use_cases_phases) > 0:
        # bar chart
        ax = use_cases_phases.plot.barh(
            color=color_cycle, stacked=True, figsize=figsize, rot=0
        )
...
def write_report_txt(report, path=None, exclude_clean=False):
    ...
    data_gen_tbl = get_table(
        phases, "Phase.DATA_GENERATION", "DATA_GENERATION", format_datetime=True
    )
    ...
    serving_throughput_tbl = get_table(
        phases, "Phase.SERVING_THROUGHPUT", "SERVING_THROUGHPUT", format_datetime=True
    )
    ...
    if bool(report["successful"]):
        ...
        for failure in report["failures"]:
            file.write("FAILURE: ")
            file.write(
                f"use case {failure['use_case']} failed during phase {failure['phase'], failure['sub_phase']}\n"
            )
