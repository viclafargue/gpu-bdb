...
for node in $nodes
do
  ssh $node 'echo "# auto-accepted EULA by $0 on `date`" >> '$PDGF_PATH'/Constants.properties'
  ssh $node 'echo "IS_EULA_ACCEPTED=true" >> '$PDGF_PATH'/Constants.properties'
done
...
do
  ...
  cmd="ssh $node cd $TPCXAI_PDGF_PATH; java -Djava.awt.headless=true $TPCxAI_PDGF_JAVA_OPTS -jar $TPCXAI_PDGF_PATH/pdgf.jar -nn $node_number -nc $node_count -ns $pdgf_options"

  echo "$cmd"
  # prefix the output with nodename
  cmd="$cmd > >(sed -e 's/^/$node: /;') 2> >(sed -e 's/^/ ERR: /;')"
  $cmd &
