...
from .data import Phase, SubPhase, SubprocessAction


class DataGeneration:
    _pdgf_init_template = Template("java -jar $pdgf_path -ns")
    _pdgf_training_template = Template(
        "java -Djava.awt.headless=true $pdgf_java_opts -jar $pdgf_path -l $schema -l $generation -ns -sf $scale_factor -sp MY_SEED $seed "
        "-sp includeLabels 1.0 -sp TTVF 1.0 -s $tables -output '\"$output/\"'"
    )

    _pdgf_training_template_parallel = Template(
        '$tpcxai_home/tools/parallel-data-gen.sh -p $pdgf_path -h nodes -o "-l $schema -l $generation -ns '
        "-sf $scale_factor -sp MY_SEED $seed -sp includeLabels 1.0 -sp TTVF 1.0 "
        '-s $tables -output \\\'\\"$output/\\"\\\'"'
    )

    _pdgf_serving_template = Template(
        "java -Djava.awt.headless=true $pdgf_java_opts -jar $pdgf_path -l $schema -l $generation -ns "
    ...
    _pdgf_serving_template_parallel = Template(
        '$tpcxai_home/tools/parallel-data-gen.sh -p $pdgf_path -h nodes -o "-l $schema -l $generation -ns '
        "-sf $scale_factor -sp SF_TRAINING $scale_factor_training -sp MY_SEED $seed "
        '-sp includeLabels 0.0 -sp TTVF $ttvf -s $tables -output \\\'\\"$output/\\"\\\'"'
    )

    _pdgf_scoring_template = Template(
        "java -Djava.awt.headless=true $pdgf_java_opts -jar $pdgf_path -l $schema -l $generation -ns "
        "-sf $scale_factor -sp SF_TRAINING $scale_factor_training -sp MY_SEED $seed "
        "-sp includeLabels 2.0 -sp TTVF $ttvf -s $tables -output '\"$output/\"'"
    )

    _pdgf_scoring_template_parallel = Template(
        '$tpcxai_home/tools/parallel-data-gen.sh -p $pdgf_path -h nodes -o "-l $schema -l $generation -ns '
        "-sf $scale_factor -sp SF_TRAINING $scale_factor_training -sp MY_SEED $seed "
        '-sp includeLabels 2.0 -sp TTVF $ttvf -s $tables -output \\\'\\"$output/\\"\\\'"'
    )
    ...
    def prepare(self):
        ...
        return SubprocessAction(
            0, cmd, Phase.DATA_GENERATION, SubPhase.INIT, metadata=None
        )
    ...
    def run(self, phase, tables):
        ...
        # training
        training_template = (
            self._pdgf_training_template
            if not self.parallel
            else self._pdgf_training_template_parallel
        )
        pdgf_train = training_template.substitute(
            pdgf_java_opts=self.pdgf_java_opts,
            tpcxai_home=self.tpcxai_home,
            pdgf_path=pdgf,
            seed=self.seed,
            schema=self.datagen_schema,
            generation=self.datagen_generation,
            scale_factor=self.scale_factor,
            has_labels=1.0,
            ttvf=1,
            tables=" ".join(tables),
            output=self.output / "training",
        )

        # serving
        serving_template = (
            self._pdgf_serving_template
            if not self.parallel
            else self._pdgf_serving_template_parallel
        )
        pdgf_serve = serving_template.substitute(
            ...
            tables=" ".join(tables),
            output=self.output / "serving",
        )

        # scoring
        scoring_template = (
            self._pdgf_scoring_template
            if not self.parallel
            else self._pdgf_scoring_template_parallel
        )
