...
DEFAULT_PHASES = [Phase.DATA_GENERATION, Phase.LOADING, Phase.TRAINING, Phase.SERVING, Phase.SERVING_THROUGHPUT,
                  Phase.SCORING_DATAGEN, Phase.SCORING_LOADING, Phase.SCORING, Phase.VERIFICATION, Phase.CHECK_INTEGRITY]
CHOICES_PHASES = [Phase.CLEAN, Phase.DATA_GENERATION, Phase.LOADING, Phase.TRAINING, Phase.SERVING,
                  Phase.SCORING_DATAGEN, Phase.SCORING_LOADING, Phase.SCORING, Phase.VERIFICATION, Phase.SERVING_THROUGHPUT,
...
def compute_tpcxai_metric(logs,SF,aiucpm_logger,datagen_in_tload=False,num_streams=2):
    ...
    #DATAGEN
    TDATA_GEN=0.0
    datagen_times=logs[(logs['sub_phase']==SubPhase.WORK) & (logs['phase']==Phase.DATA_GENERATION)][['phase','sub_phase','metric']]
    ...
    loading_times=logs[(logs['sub_phase']==SubPhase.WORK) & (logs['phase']==Phase.LOADING)][['phase','sub_phase','metric']]
...
def main():
    ...
    for uc_key in use_cases:
        ...
        scoring_download_cmd = serving_datastore.download.safe_substitute(source=scoring_source, destination=model_url)
    ...
    if Phase.SERVING_THROUGHPUT in phases:
        ...
        if dry_run:
            ...
            for thread, stream in zip(stream_threads, streams):
                for result, action, duration in zip(thread.results, stream, thread.timings):
                    if result:
